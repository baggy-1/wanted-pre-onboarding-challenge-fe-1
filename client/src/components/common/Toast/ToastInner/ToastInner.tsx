...
const ToastInner = ({ children, ...props }: Props) => {
  ...
  return (
    <div
      className="w-80 h-auto min-h-[4rem] bg-white rounded-[4px] p-2 flex items-center justify-start shadow-lg relative"
      onClick={handleCloseToast}
      ref={toastRef}
    >
      <div>{iconOut}</div>
      <div className="p-[6px] break-keep">{children}</div>
      <div
